...
const {findByEmail, findById, updateUser} = require('../repositories/userRepository')
...
class UserService{
    ...
    static forgotPassword = async ({ email }) => {
        try {
            ...
            const expireTime = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Ho_Chi_Minh" }))
    ...
    static resetPassword = async ({ email, resetCode, newPassword }) => {
        try {
            const checkUser = await userModel.findOne({ email })
            if (!checkUser) {
                throw new NotFoundError('email not found')
    ...
    static updatePr = async ({ user, updateData }) => {
        ...
        return await updateUser({ user, updateData });
      };
