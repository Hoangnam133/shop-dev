...
const getProductStockInAllShops = async ({product_id, limit = 10, page = 1}) => {
    ...
    const stockData = await inventoryModel.find({ product_id, isDelete: false })
        .populate('shop_id')
        .populate('product_id')
        .skip(skip)
        .limit(limit)
...
const getLowStockProductsInShop = async ({shop_id, limit = 10, page = 1}) => {
    ...
    if (!lowStockProducts || lowStockProducts.length === 0) {
        throw new BadRequestError('There are no out of stock products in this branch');
    }
...
const getProductStockInShop = async ({shop_id, product_id, limit = 10, page = 1}) => {
    ...
    const skip = (page - 1) * limit;
    const stockData = await inventoryModel.findOne({ product_id, shop_id })
        .populate('shop_id')        
        .populate('product_id')
        .skip(skip)
        .limit(limit)     
    ...
    if (!stockData) {
        throw new BadRequestError('No products in stock were found for this branch');
    }
    return stockData
}
// isDelete 
const softDeleteProductInInventory = async ({ shop_id, product_id }) => {
    ...
    if (!updatedInventory) {
        throw new BadRequestError('No products found in this branch warehouse to delete');
    }
...
const addProductToInventory = async ({ shop_id, product_id, quantity, minStockLevel }) => {
    ...
    const existingInventory = await inventoryModel.findOne({ shop_id, product_id });
    if (existingInventory) {
        throw new BadRequestError('The product already exists in this branch warehouse')
    }
    if (quantity < 0) {
    ...
    if (quantity < minStockLevel) {
        throw new BadRequestError('The inventory quantity must be greater than or equal to the minimum inventory level')
    }
...
const updateInventory = async ({ shop_id, product_id, quantity, minStockLevel }) => {
    ...
    const updatedInventory = await inventoryModel.findOneAndUpdate(
        { shop_id, product_id },
        { inven_stock: quantity, minStockLevel },
        {
            new: true,   
            lean: true
        }
    ...
    if (!updatedInventory) {
        throw new BadRequestError('No products found in this branch warehouse for update')
    }
...
const checkProductOutOfStockAllShops = async ({product_id, limit, page}) => {
    ...
    const skip = (page - 1) * limit;
    const outOfStockShops = await inventoryModel.find({ product_id, inven_stock: 0 })
        .populate('shop_id')
        .populate('product_id')
        .skip(skip)
        .limit(limit)
...
const getDeletedProductsInInventory = async ({shop_id, limit = 10, page = 1}) => {
    ...
    const skip = (page - 1) * limit;
    const deletedProducts = await inventoryModel.find({ 
        isDelete: true,  
        ...(shop_id && { shop_id })  
    })
    .populate('shop_id')
